...
class Othello25:
    ...
    def update(self):
        ...
        if self.scene == "TITLE":
            if pyxel.btnp(pyxel.KEY_1) or (pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT) and pyxel.mouse_x < SCREEN_SIZE // 3):
                self.difficulty = 1
                self.scene = self.scene = "GAME"
                ...
                if js:
                    try:
                        ...
                        pass
                pyxel.playm(0, loop=True)
            elif pyxel.btnp(pyxel.KEY_3) or (pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT) and pyxel.mouse_x >= (SCREEN_SIZE // 3) * 2):
                self.difficulty = 3
                self.scene = self.scene = self.scene = "GAME"
    ...
    def draw(self):
        pyxel.cls(0)
        if self.scene == "TITLE":
            pyxel.text(5, 5, "ATK 3MOKU", pyxel.frame_count % 16)
            pyxel.text(12, 18, "SELECT", 7)
            pyxel.text(5, 28, "1   2   3", 7)
            pyxel.text(5, 38, "TAP OR KEY", 7)
        else:
